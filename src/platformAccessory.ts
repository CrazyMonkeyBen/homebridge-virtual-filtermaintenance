...
export class VirtualFilterAccessory {
	...
	constructor (private readonly platform: VirtualFilterPlatform,
		...
		this.accessory.getService(this.platform.Service.AccessoryInformation)!
			...
			.setCharacteristic(this.platform.Characteristic.Model, accessory.context.deviceConfig['device-type'] + " - " + accessory.context.deviceConfig['duration'])
		...
		this.platform.log.debug(this.accessory.displayName, durationString, " year:", years, " month:", months, " day:", days, " hour:", hours);
	...
	calculateFilterLife () {
		...
		let elapsedTime = currentTime - this.accessory.context.startTime;
		...
		this.platform.log.debug(this.accessory.displayName, 'Calculate Filter: Percent -> ', percent, " Expired ", expired);

		return [expired, percent];
	}

	calculateDuration () {
		let startTimeMS = this.accessory.context.startTime;

		let endTime = new Date(startTimeMS);
